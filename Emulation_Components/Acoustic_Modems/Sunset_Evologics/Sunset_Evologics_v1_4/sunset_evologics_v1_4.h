...
#ifndef __Sunset_Evologics_v1_4_h__
#define __Sunset_Evologics_v1_4_h__

#include "sunset_evologics_include.h"
#include "sunset_evologics_connection.h"
#include "sunset_evologics_def.h"

#define EV_ATT_REQUEST_TIME_1_4 	0.2

class Sunset_Evologics_v1_4;
...
public:
	Sunset_Evologics_Timeout_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_DeliveryStatus_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_BurstMsgResp_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_RangingReply_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_RTT_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_Start_Connection(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
public:
	Sunset_Evologics_Rx_Timer(Sunset_Evologics_v1_4 *m) : Sunset_Generic_ModemTimer((Sunset_Generic_Modem*)m) { }
...
/*! @brief This class implements the  Evologics Modem (firmware version 1.4) driver. It extends the generic modem class.
 * @see class Sunset_Generic_Modem
 */

class Sunset_Evologics_v1_4 : public Sunset_Generic_Modem {
public:
	...
	void RxIterate(Sunset_Evologics_Conn *);
...
protected:
	...
	void checkSetting();	//control if other modem settings have to be performed
...
public:
	...
	bool send_data_im(char *, int, int, char *);	//send Instant Message
	bool send_data_imRTT();		//send ranging request
	bool send_data_burst(char *, int, int);	//send burst of data
	
	virtual void rxAborted();	//error on reception
	
	virtual void txDone();	//transmission completed
	
	virtual void txAborted();	//error on transmission
	
	void rxDoneIm(char *, int);	//receive instant message
	void rxDoneBurst(char *, int);	//receive burst of data
	void rxFailed(char *, int);	//error on reception
	
	void modemTimeout();		//timeout on modem operation
	
	void modemDeliveryTimeout();	//timeout when checking about modem delivering operation
	void modemBurstMsgRespTimeout();	//timeout on burst data transmission
...
protected:
	...
	bool modem_setting();			//perform the setting of the modem
...
void *thread_proxy_function_1_4(void *);

...
load $pathMiracle/libmiracleport.so.0.0.0
load $pathMiracle/libUwmStd.so.0.0.0
...
load $pathSUNSET/libSunset_Networking_Phy_Bellhop.so.0.0.0  
...
set phyPreambleTime 10.0 ;# we assume a preamble of 10 ms at th ephysical layer for training and signal detection 
...
if { $params(urick) == 1 } {
	puts "Script designed to run in simulation mode using Bellhop channel model, while bellhop model has been selected ERROR"
	exit 1
}  
############################################################
...
if {$params(bellhop) == 1} {
	if { $params(use_db) == 1 } {
		source "./tcl_folder/SUNSETBellhopFileDb.tcl"
	} else {
		source "./tcl_folder/SUNSETBellhopFileNoDb.tcl"
	}
}
...
proc createNode { id }  {
	...
	global phy params mac_ woss_utilities source_  routing_ cbr_ channel_estimator woss_creator
	...
	set position_($id) [new "WOSS/Position"]
	...
	$ch_estimator_plugin($id) insertNode [$mac_($id) addr] $position_($id)
	$node_($id) addPlugin $ch_estimator_plugin($id) 19 "CHE"

	$woss_creator setCustomAngles $position_($id) 0 $params(node_min_angle) $params(node_max_angle)
...
proc createSink { } {
	global channel propagation data_mask ns  position_ node_ portnum_ energy
	global phy params mac_ woss_utilities source_  routing_ cbr_sink_ portnum_sink_ channel_estimator woss_creator
	...
	set phy($params(sink)) 		[new "Module/Sunset_Phy_Bellhop"]
	...
	set position_($params(sink)) [new "WOSS/Position"]
	...
	set ch_estimator_plugin($params(sink)) [ new "WOSS/PlugIn/ChannelEstimator"]
	$ch_estimator_plugin($params(sink)) setChannelEstimator $channel_estimator
	$ch_estimator_plugin($params(sink)) insertNode [$mac_($params(sink)) addr] $position_($params(sink))
	$node_($params(sink)) addPlugin $ch_estimator_plugin($params(sink)) 19 "CHE"

	$woss_creator setCustomAngles $position_($params(sink)) 0 $params(node_min_angle) $params(node_max_angle)
...
proc startModule { } {
	...
	global ns  params time START_DELAY  source_  routing_ mac_  info_dispatcher energy phy statistics
